...
public class AppServer extends SyncPrimitive implements Runnable, ReplicatedLog.LogReplicator {
		...
		for (int i = 1; i < nodeList.size(); i++) {
			...
			try {
				if (followerChannel == null) {
					...
					AccountServiceGrpc.AccountServiceBlockingStub blockingStub = AccountServiceGrpc
	...
	public void replicateOnFollowers(Long entryAtIndex, byte[] data) {
		LogEntry logEntry = LogEntry.newBuilder().setEntryAtIndex(entryAtIndex)
				.setLogEntryData(ByteString.copyFrom(data)).build();
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		if (args.length != 3) {
			System.out.println(
					"Usage java -cp nes123-0.0.1-SNAPSHOT.jar;.;lib/* ostalo.AppServer <zookeeper_server_host:port> <gRPC_port> <log_file_name>");
			System.exit(1);
		}
		...
		Server gRPCServer = ServerBuilder.forPort(gRPCPort).addService(new ServiceGRPCServer(accService, node)).build();
